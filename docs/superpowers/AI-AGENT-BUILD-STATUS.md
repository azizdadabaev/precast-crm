...
**Last updated:** 2026-06-08 (Plans 01–08 done; Plan 09 Slice A + B done; **Slice C Steps 3–5 BUILT — Suggest (operator Send/Edit), Order-taking (operator places real Orders), Auto (replies auto-send; orders/escalations always → human). All 3 modes selectable in /agent.** KB tuned; get_quote capped at 6.30m beam. **The feature-completion plan (0–5) is code-complete.** Remaining: PROD DEPLOY (owner) + parallel items (voice/vision/KB-editor/few-shot/eval). See "▶ RESUME HERE".)
...
5. ✅ **DONE (2026-06-08)** — Auto mode (`mode==='auto'`). `auto-mode.ts`: `autoActionFor` (reply→send, else→human) + `applyAutoMode` + `routeToHuman` (PENDING_HUMAN + AGENT_ESCALATION notify to inbox.access owners) + `defaultAutoModeDeps`. `webhook-entry` auto-sends a `reply` and routes everything else to a human; **orders are NEVER auto-placed** (request_approval → operator places in /inbox). All 3 modes selectable in `/agent`. 5 tests incl. the safety invariants.
   ▶ **REMAINING — PROD DEPLOY (owner action)** — deploy to prod (standard deploy in CLAUDE.md), then graduate per spec §14: turn the kill-switch ON in shadow → watch the persisted `AgentProposal` rows + the 3-model bake-off → suggest → limited auto → full auto. ⚠️ Re-verify model ids/prices + pin `requiresSnapshotPin` snapshots first; confirm prod has provider keys + `QUOTE_SIGNING_SECRET` + the KB loaded.
Parallel/later: ✅ **KB editor UI DONE** (2026-06-08 — `/agent` Knowledge base section + `GET/PUT /api/agent/kb` + `kb-lint.ts`; spec §9). ✅ **Floor-plan VISION DONE** (2026-06-08 — `provider.extractDimensions` + Gemini impl + `parseDimensions` + `vision.ts` echo-to-confirm + `runVisionForInbound`/webhook step 8b + Simulate image-attach; inbound photo → read dims → echo to confirm, never quotes off a sketch, always human-reviewed, spec §4.5). ✅ **VOICE transcription DONE** (2026-06-08 — `runVoiceForInbound`/webhook step 8c: inbound voice note → Gemini `transcribe()` → stored transcript → agent → proposal, always human-reviewed; shared `generateAndPersistProposal` core; Simulate audio-attach). ✅ **FEW-SHOT tuning DONE** (2026-06-08 — `loadFewShot` injected in the shared agent-run core + test console; `/agent` few-shot editor via a generalized `AgentDocEditor`; `GET/PUT /api/agent/fewshot`; TONE-guide only). **Remaining parallel item: eval / 3-model bake-off** (golden set + Claude vs Gemini vs OpenAI scoring over the persisted `AgentProposal` rows — spec §3/§14; most useful right before/around prod deploy).
...
| 09 | **Operator UI + rollout: control panel + API keys + test console (DONE) · inbox ghost-drafts · KB editor · write-action/auto-send activation · voice/vision · bake-off** | 🚧 Slice A + B DONE · Slice C 3/4/5 (Suggest+Order-taking+Auto) DONE · remaining: prod deploy + parallel items |
