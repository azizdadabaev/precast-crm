...
Parallel/later: ✅ **KB editor UI DONE** (2026-06-08 — `/agent` Knowledge base section + `GET/PUT /api/agent/kb` + `kb-lint.ts` price-warning; owner edits the KB in-app, spec §9). Remaining: voice (`gemini.transcribe()`), vision (drawing photos), few-shot wiring (curated+anonymized Telegram/IG chats as a TONE guide only), eval/bake-off.
