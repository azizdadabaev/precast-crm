...
- **R8 — DeliveryLocation's map stays.** The Google Maps composable and `DeviceLocation` are untouched; the controls (link field, manual coordinates, «Менинг жойим», label, Save/Clear) become `FormCard` fields + `EtalonIconButton`s; without the API key the tile is blank as today (owner note).
