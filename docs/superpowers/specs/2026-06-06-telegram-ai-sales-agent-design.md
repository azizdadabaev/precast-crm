...
**Candidates available in the testing stage (all three wired behind the same interface).** Verified ids/prices live in the registry — [`src/lib/agent/llm/models.ts`](../../../precast-crm/src/lib/agent/llm/models.ts) (`PRICING_VERIFIED_AT`); re-verify + pin dated snapshots before Shadow. As of 2026-06-07:
- **Anthropic Claude — Opus 4.8** (`claude-opus-4-8`, $5/$25 per MTok, 1M context, native vision; also Sonnet 4.6 `claude-sonnet-4-6` $3/$15 as a cheaper brain). Strengths: best *measured* Uzbek score (Claude lineage leads the older TUMLU benchmark), strongest agentic tool-calling reliability, Opus-4.7+ mid-conversation system re-anchoring for jailbreak defense. **No raw-audio input** (so never the STT). Pin a **dated snapshot**, not the alias.
- **Google Gemini — 3.1 Pro** (`gemini-3.1-pro-preview`, $2/$12, 2M context, vision + native audio) and **3.5 Flash** (`gemini-3.5-flash`, $1.50/$9, vision + native audio, low latency). The 3.x line supersedes the 2.5 line. Strengths the owner values: strong image understanding and **native audio** (voice notes), competitive Uzbek/Russian, lower cost. (Pro id is a preview — pin a stable id before Shadow.)
- **OpenAI GPT — 5.5** (`gpt-5.5`, $5/$30, ~1.05M context) and **5.4** (`gpt-5.4`, $2.50/$15). Included for completeness; confirm exact id/price at wiring time (vendor naming varies).

**Fixed regardless of which wins the "conversation brain":**
- **Voice-note transcription = Google Gemini** (native Uzbek/Russian audio) — primary candidate `gemini-3.5-flash` (native audio, cheap), with `gemini-3.1-flash-live-preview` as a real-time alt. Claude's API cannot accept raw audio, so audio is transcribed to text *before* the agent loop. (We can A/B a dedicated STT too.)
- **Image (floor-plan) reading** is a multimodal step we will benchmark across Claude vision (Opus 4.8 / Sonnet 4.6) and Gemini vision (3.1 Pro / 3.5 Flash) on real sketches; Gemini is a strong candidate here per the owner's instinct.
- **Input-screen classifier** stays a small/cheap model — Claude Haiku 4.5 (`claude-haiku-4-5-20251001`, $1/$5), Gemini 3.1 Flash-Lite (`gemini-3.1-flash-lite`, $0.25/$1.50), or GPT-5 Mini (`gpt-5-mini`, $0.25/$2) — injection/off-topic screen, ~$0.0006/msg.
