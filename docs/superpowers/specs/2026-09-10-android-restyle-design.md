...
| Етказиш (timeline, Сана · Ҳайдовчи, location button) | Deliver | Always | **Three steps (owner, 2026-09-11): «Буюртма» → «Юкланди» → «Етказилди»**, the web's ЖАРАЁН. PLACED/IN_PRODUCTION sit on step 1 (`placedAt`), LOADED/DISPATCHED on step 2 (the ORDER_LOADED event or the earliest shipment `loadedAt`), DELIVERED on step 3 (`deliveredAt`); step 2 needs the truck photo and step 3 the money in, which the load-truck and delivery-proof flows enforce. `dispatch`, `shipments[]`, `deliveryLat/Lng/Url` for the footer and the location button. The server's seven statuses stay on tags and chips because orders in the database carry them. |
