...
class PushRegistrar @Inject constructor(private val devices: DeviceRepository) {
     ...
     *  throws, so the whole lookup — not just the token task — sits inside runCatchingCancellable,
     *  which re-throws CancellationException instead of swallowing it. */
    suspend fun registerIfPossible() {
        val token = runCatchingCancellable { FirebaseMessaging.getInstance().token.await() }.getOrNull() ?: return
