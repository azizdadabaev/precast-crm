...
We want a **self-contained, to-scale, branded drawing sheet** — title block + dimensioned plan + BoM/cut-list + totals + weight — generated **in-house on the server**, always available, that a client can act on and a builder can build from. This also lets us **retire the Blender bridge to a fallback**.
...
| Decision | Choice |
|---|---|
| PDF production | **Server-side render route** — `puppeteer-core` (already a dep) drives headless Chromium to print an internal print page to PDF at exact A4 mm. Perfect Cyrillic fonts + true mm scale, and doubles as the always-on drawing-on-demand that retires the Blender bridge. |
...
| Bridge | Keep the Blender WebSocket bridge as a **legacy fallback** behind the new in-house render. |
...
- `weight.ts` — `estimateWeight(calc, consts)` = beams (Σ length × kg/m) + blocks (× kg/block) + topping (concrete_volume × density). Constants from `AppConfig` (confirmed with owner; until then render a clearly-labelled estimate or hide).
...
- `DrawingsSection` / `SendToBlenderButton` call this **first**; Blender bridge becomes the fallback.
...
- **Weight gating:** never imply a crane size from an unverified number — gate the weight line behind owner-confirmed constants.
...
4. **Weight** (`weight.ts`) — gated on owner constants.
5. **Branding + Uzbek-first styling** (header band, `useT()` localization of sheet strings, phone-legible fonts).
6. **Multi-room project sheet** (`sheet-pack.ts` + grand BoM via `mergeBeamSchedule`, grand total via `projectTotal`).
7. **Wire render route as primary**, Blender bridge as fallback; verify always-available.
...
- **Chromium in prod Docker:** image size + a bundled Cyrillic font (e.g. DejaVu/Noto). Must be added to the Dockerfile; verify `puppeteer-core` finds the binary.
- **Weight constants** (beam kg/m by section, filler-block kg) are **not in the repo** — Phase 4 blocked until the owner confirms; ship a labelled estimate or hide until then.
- **RoomCanvas duplication:** the sheet must reuse the pure overlay producers, not fork the rendering — keep the sheet a thin composable layer.
...
- **Open for owner:** weight constants; company header content (name/logo/phone); A4 vs A3 default; does the sheet render only from a **saved** `Calculation`, or also from transient calculator state (signed payload)?
