...
/** PATCH /api/gazoblok/orders/[id]/comments/[cid] — author only, 30-min window */
export const PATCH = withAuth<Params>(
  async (req: NextRequest, { params, user }) => {
    ...
    if (!existing || existing.gazoblokOrderId !== params.id) {
...
/** DELETE /api/gazoblok/orders/[id]/comments/[cid] — author OR comment.moderate */
export const DELETE = withAuth<Params>(
  async (_req: NextRequest, { params, user }) => {
    const existing = await prisma.comment.findUnique({
      where: { id: params.cid },
      select: { id: true, gazoblokOrderId: true, authorId: true, deletedAt: true },
    });
    if (!existing || existing.gazoblokOrderId !== params.id) {
      return fail("Comment not found", 404);
    }
