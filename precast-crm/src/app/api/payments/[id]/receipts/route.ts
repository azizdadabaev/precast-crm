...
export const POST = withPermission<{ id: string }>("payment.record", withIdempotency<{ id: string }>(async (req: NextRequest, { user, params }) => {
  ...
  if (!file || typeof file !== "object" || !("arrayBuffer" in file)) return fail("Расм юборилмади · No file provided", 422);
  const f = file as File;
  if (!ALLOWED_IMAGE_MIME.has((f.type || "").toLowerCase())) return fail("Фақат JPG, PNG ёки WEBP расм қабул қилинади · Only JPG, PNG, or WEBP images are accepted", 422);
  if (f.size === 0) return fail("Файл бўш · Empty file", 422);
  if (f.size > MAX_IMAGE_SIZE_BYTES) return fail("Расм жуда катта (кўпи билан 8 МБ) · Image too large (max 8 MB)", 413);
  const buffer = Buffer.from(await f.arrayBuffer());
  const ext = imageExtFromBytes(buffer);
  if (!ext) return fail("Бу расм эмас · Not a valid image", 422);
